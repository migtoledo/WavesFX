 ...
 * VirtualSystem ]]]
 * <p>
 * FX2 : Waves System : Virtual System
 */
public final class FX2 {
    ...
    double f(double x) {
        double y = GlobalsFX2.A2 * Math.cos(GlobalsFX2.K2 * x - GlobalsFX2.W2*GlobalsTimers.t);
        return y;
    }
    ...
    void computeParticlesMatrix() {
        reloadParticles();
        computeXY();
        System.out.println("Computing particles of F2 : " + particlesMatrix.size());
    }
    ...
    void calculateNewPointsXY() {
        ...
        for (double x = GlobalsXY.xMin; x < GlobalsXY.xMax; x += GlobalsFX2.dx2) {
            particlesMatrix.add(new PointXY(new XY(x, f(x)), Color.GREEN));
        }
    }
